...
class CustomAtariPreprocessing(gym.Wrapper, gym.utils.RecordConstructorArgs):
    ...
    def __init__(
        ...
        assert isinstance(env.observation_space, (Box, gym.spaces.Dict))
        if isinstance(env.observation_space, Box):
            ...
            if grayscale_obs:
                ...
                self.obs_buffer = [
                    np.empty(env.observation_space["rgb"].shape, dtype=np.uint8),
                    np.empty(env.observation_space["rgb"].shape, dtype=np.uint8),
                ]
            self.lives = 0
            self.game_over = False
            ...
            self.observation_space = gym.spaces.Dict({
                "rgb": Box(low=_low, high=_high, shape=_shape, dtype=_obs_dtype),
                "objects_position": Box(low=-(2**63), high=2**63 - 2, shape=(len(env.observation_space["objects_position"].low),), dtype=np.float32)
            })
